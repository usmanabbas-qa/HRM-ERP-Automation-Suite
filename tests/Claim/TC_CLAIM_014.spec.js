...
test('TC_Claim_014: Verify Viewing Details of a Claim from Employee Claims Page', async () => {
    await claim.clickClaimTab();
    await claim.verifyClaimPage();
    await adminOrganization.clickTopMenu('Employee Claims');
    await claim.clickViewDetailButton();
    await adminJob.verifyPageTitle("Submit Claim");
});